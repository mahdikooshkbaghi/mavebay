...
from ._npeet import entropy, mi, micd


def entropy_continuous(
    x, knn=5, resolution=0.01, uncertainty=True, num_subsamples=25, verbose=False
):
    """
    ...
    # If user does request uncertainty, do computations
        ...
        for k in range(num_subsamples):
            ...
            x_k = x[ix, :]
            H_subs[k] = entropy(x_k, k=knn, base=2)
...
def mi_mixed(
    ...
    # Get best I estimate
    I_best = micd(x, y, k=knn, warning=warning)
    ...
    if uncertainty:
        ...
        for k in range(num_subsamples):
            ...
            x_k = x[ix, :]
            y_k = y[ix, :]
            I_subs[k] = micd(x_k, y_k, k=knn, warning=False)
    ...
    if verbose:
        print(f"Arguments: knn={knn}, num_subsamples={num_subsamples}")
        print(f"Results: I={I_best:.4f} bits, dI={dI:.4f} bits")
...
def mi_continuous(
    ...
    # Get best I estimate
    I_best = mi(x, y, k=knn, alpha=alpha_LNC)
    ...
    if uncertainty:
        ...
        for k in range(num_subsamples):
            ...
            x_k = x[ix]
            y_k = y[ix]
            I_subs[k] = mi(x_k, y_k, k=knn, alpha=alpha_LNC)
...
def I_intrinsic(y_values, dy_values, verbose=False):
    """
    ...
    Hs_bits = 0.5 * np.log2(2 * pi * e * dy**2)
    N = len(Hs_bits)
    H_ygx = np.mean(Hs_bits)
    dH_ygx = np.std(Hs_bits, ddof=1) / np.sqrt(
        N
    )  # Note that need to specify 1 DOF, not that it matters much.
